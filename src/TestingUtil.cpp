...
bool InitTest(const std::string& dirPath,
    ...
    std::string root = tmp_path.str();
    std::string appname = "clang-xform";
    auto pos = root.find(appname);
    if (pos == std::string::npos) return false;
    
    root = root.substr(0, pos + appname.length());
    // cd to clang-xform root directory
    fs::set_current_path(root + '/' + dirPath);
